...
def list_edges(
    ...
    if edge_nature:
        sql += " AND e.edge_nature = %s"
        params.append(edge_nature)
    if subject:
        sql += " AND (ofrom.subject = %s OR oto.subject = %s)"
...
def update_edge(from_occurrence: int, to_occurrence: int, body: EdgeUpdate) -> dict:
    ...
    try:
        with conn.cursor(cursor_factory=RealDictCursor) as cur:
            ...
            cur.execute("""
                UPDATE edges SET edge_nature = %s, confirmed_by = %s, confirmed_date = %s
                WHERE from_occurrence = %s AND to_occurrence = %s
            """, (
                body.edge_nature,
